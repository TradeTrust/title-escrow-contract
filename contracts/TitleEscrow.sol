...
contract HasHolder is Context {
  ...
  function _changeHolder(address newHolder) internal {
    require(
      newHolder != address(0),
      "HasHolder: new holder is the zero address"
    );
...
contract TitleEscrow is Context, HasNamedBeneficiary, HasHolder, ITitleEscrow {
  ...
  function changeHolder(address newHolder) external isHoldingToken onlyHolder {
  ...
  modifier isHoldingToken() {
    ...
    require(
      tokenRegistry.ownerOf(_tokenId) == address(this),
      "TitleEscrow: Contract is not the owner of token"
    );
    _;
  }

  function endorseTransfer(address newBeneficiary)
    external
    isHoldingToken
    onlyBeneficiary
  {
  ...
  function transferTo(address newBeneficiary)
    external
    isHoldingToken
    onlyHolder
  {
